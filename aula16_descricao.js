...
function validarProduto({ nome, preco, marca, estoque, descricao }) {
  ...
  // Estoque: obrigatório, numérico inteiro, maior ou igual a zero
...
app.get("/api/produtos/", (req, res) => {
  ...
  // 3. Filtros por estoque
  if (estoque_minimo !== undefined && estoque_minimo !== "") {
    resultado = resultado.filter((p) => p.estoque >= parseInt(estoque_minimo));
  }
  if (estoque_maximo !== undefined && estoque_maximo !== "") {
    resultado = resultado.filter((p) => p.estoque <= parseInt(estoque_maximo));
  }
  ...
  // 6. total_pages calculado sobre o total já filtrado/pesquisado/ordenado
  const totalParaPaginacao = resultado.length;
  const totalPages = Math.ceil(totalParaPaginacao / tamanhoPagina);

  // 7. Aplica o corte da página (slice)
